...
accounts ──< account_sessions                     (one per sign in, revocable)
accounts ──< api_usage_logs                       (one per authenticated request)
...
| `account_sessions` | One row per credential that can authenticate: a browser sign in (`SESSION`) or a machine token (`API`). They share a table because every endpoint treats them identically and they differ in only two ways, how long they live and how they are made, so revocation and listing are written once. Many per account by design, since one person is signed in on several devices at once. The row is the authority, which is what makes revocation possible at all: a signed token cannot be taken back, so signing out, ending one device and "sign out everywhere else" all work by marking a row rather than by hoping a client forgets something. Stores a SHA-256 digest, never the token. |
| `api_usage_logs` | One row per authenticated request: method, path, status and duration. The highest volume table in the schema, so it is written in batches off the response path and purged on a retention window. `session_id` is a plain column rather than a foreign key, because the record of what a credential did has to outlive the credential. Deliberately holds no body, headers, query string or address: there is nowhere to put one. |
